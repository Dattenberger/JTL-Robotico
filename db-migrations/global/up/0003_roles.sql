...
--                        They get EXECUTE on reset.spPub_StartTestmandantReset /
--                        reset.spPub_GetResetStatus (granted in permissions/100_grants).
--                        They are DENIED the cShopLicense column so a reset operator
--                        can never read a mandant's shop license key.
--   ops_admin          — full control of the ops registry (maintain ops.tMandant /
--                        ops.tConfig). Membership is assigned by a human, out of band.
--
...
GRANT SELECT, INSERT, UPDATE, DELETE ON ops.tMandant TO ops_admin;
GRANT SELECT, INSERT, UPDATE, DELETE ON ops.tConfig  TO ops_admin;
...
-- exclusively through reset.spPub_PurgeOldRequests (ownership-chained), so no direct DELETE is
-- granted — the audit trail cannot be casually erased.
GRANT SELECT, UPDATE ON ops.tResetRequest TO ops_admin;

-- Reset operators must never see license keys. spPub_GetResetStatus already omits the
-- column; this column-level DENY is defense in depth in case someone grants the
-- role broader SELECT later.
DENY SELECT ON ops.tMandant (cShopLicense) TO ops_reset_executor;
GO
