-- 0022_maintenance_ola_vendor.sql  (Ebene B / global chain -- runs in RoboticoOps)
--
-- Vendors the PINNED Ola Hallengren maintenance objects into RoboticoOps.dbo:
--   dbo.CommandLog (table), dbo.CommandExecute, dbo.DatabaseIntegrityCheck,
--   dbo.IndexOptimize (procedures).
--
-- Pinned upstream version: 2026-07-22 20:03:34 (the '--// Version:' stamp inside each
-- proc; source https://github.com/olahallengren/sql-server-maintenance-solution,
-- release tag 20260722_200334). Upgrade = a NEW up/ script (this file is immutable
-- once applied). The four SINGLE-OBJECT scripts are vendored instead of the bundled
-- MaintenanceSolution.sql: the bundle would need its @CreateJobs header edited --
-- exactly the edit "vendored unchanged" forbids.
--
-- dbo.DatabaseBackup is DELIBERATELY NOT vendored: backups stay with CBB (ADR-B).
-- What is not deployed cannot be accidentally scheduled.
--
-- Idempotency: upstream already guards each object (IF NOT EXISTS create-stub +
-- ALTER PROCEDURE; CommandLog's CREATE TABLE is IF-NOT-EXISTS-wrapped upstream since
-- the pinned version -- the FT-13 wrapper from the plan is therefore not needed).
--
-- Documented byte-deviations from upstream (all marked VENDOR-DEVIATION inline):
--   * three '1900-01-01' dashed date literals rewritten to '19000101' (lint rule (h);
--     a dashed literal is reparsed under DATEFORMAT dmy on the German-locale servers).
--   * CRLF line endings normalized to LF, per-file BOMs stripped (concatenation).
--
...
-- ============================================================================
-- VENDORED: CommandLog.sql (Ola Hallengren, Version 2026-07-22 20:03:34)
-- ============================================================================
SET ANSI_NULLS ON
GO
...
-- ============================================================================
-- VENDORED: CommandExecute.sql (Ola Hallengren, Version 2026-07-22 20:03:34)
-- ============================================================================
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
...
BEGIN
  ...
  -- VENDOR-DEVIATION (lint rule (h), documented byte-break): the upstream line below used the
  -- ambiguous dashed literal '1900-01-01'; rewritten to the language-neutral basic ISO form.
  -- @see https://github.com/olahallengren/sql-server-maintenance-solution (Version: 2026-07-22 20:03:34)
  SET @EndMessage = 'Duration: ' + CASE WHEN (DATEDIFF(SECOND,@StartTime,@EndTime) / (24 * 3600)) > 0 THEN CAST((DATEDIFF(SECOND,@StartTime,@EndTime) / (24 * 3600)) AS nvarchar(max)) + '.' ELSE '' END + CONVERT(nvarchar(max),DATEADD(SECOND,DATEDIFF(SECOND,@StartTime,@EndTime),'19000101'),108)
  RAISERROR('%s',10,1,@EndMessage) WITH NOWAIT
...
-- ============================================================================
-- VENDORED: DatabaseIntegrityCheck.sql (Ola Hallengren, Version 2026-07-22 20:03:34)
-- ============================================================================
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
...
BEGIN
  ...
  BEGIN
    UPDATE tmpDatabases
    -- VENDOR-DEVIATION (lint rule (h), documented byte-break): the upstream line below used the
    -- ambiguous dashed datetime literal '1900-01-01 00:00:00.000'; rewritten to the
    -- language-neutral basic ISO form.
    -- @see https://github.com/olahallengren/sql-server-maintenance-solution (Version: 2026-07-22 20:03:34)
    SET LastGoodCheckDbTime = NULLIF(CAST(DATABASEPROPERTYEX (DatabaseName,'LastGoodCheckDbTime') AS datetime2),'19000101 00:00:00.000')
    FROM @tmpDatabases tmpDatabases
  END
...
-- ============================================================================
-- VENDORED: IndexOptimize.sql (Ola Hallengren, Version 2026-07-22 20:03:34)
-- ============================================================================
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            -- ambiguous dashed literal '1900-01-01'; rewritten to the language-neutral basic ISO form.
            -- @see https://github.com/olahallengren/sql-server-maintenance-solution (Version: 2026-07-22 20:03:34)
            SET @CurrentDelay = DATEADD(ss,@Delay,'19000101')
