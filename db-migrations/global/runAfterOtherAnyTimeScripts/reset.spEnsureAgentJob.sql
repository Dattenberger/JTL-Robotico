-- reset.spEnsureAgentJob  (Ebene B / global — runAfterOtherAnyTimeScripts, anytime)
--
...
-- the job, research/3 §3), one step "EXEC reset.spProcessNextResetRequest", enabled,
-- NO schedule (on-demand only, kicked by reset.spPub_StartTestmandantReset -> sp_start_job).
--
-- OPS-4: failure notification is OPTIONAL and config-gated. A failed/stale-reclaimed
-- reset is otherwise pull-only (nobody is told unless they poll reset.spPub_GetResetStatus).
-- If the admin seeds ops.tConfig('NotifyOperator') with an existing SQL-Agent operator
...
-- reset.spProcessNextResetRequest already exists. Idempotent: drop-if-exists then add.
--
...
-- self-executing wrapper proc (named reset.spEnsureAgentJob) instead. A lint gap was
...
CREATE OR ALTER PROCEDURE reset.spEnsureAgentJob
AS
BEGIN
    SET NOCOUNT ON;

    -- Single-sourced job name (CQG-8): the same ops.tConfig knob is read here, in
    -- reset.spPub_StartTestmandantReset (sp_start_job) and in permissions/200_ensure_agent_job
    ...
    DECLARE @notifyOperator sysname =
        NULLIF((SELECT cValue FROM ops.tConfig WHERE cKey = N'NotifyOperator'), N'');
    ...
    EXEC msdb.dbo.sp_add_job
         ...
         @description = N'On-demand test-mandant reset. Started by reset.spPub_StartTestmandantReset via sp_start_job.',
    ...
    EXEC msdb.dbo.sp_add_jobstep
         ...
         @command = N'EXEC reset.spProcessNextResetRequest;',
         @retry_attempts = 0,
